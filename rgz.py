...
rgz.config['SQLALCHEMY_DATABASE_URI'] = f'postgresql://{user_db}:{password}@{host_ip}:{host_port}/{database_name}'
rgz.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
db.init_rgz(rgz)

login_manager = LoginManager(rgz)

login_manager.login_view = "login"
login_manager.init_rgz(rgz)
...
@rgz.route('/create_book', methods=['GET', 'POST'])
@login_required
def create_book():
    if request.method == 'POST':
...
@rgz.route('/edit_book', methods=['GET', 'POST'])
@login_required
def edit_book():
...
@rgz.route('/delete_book', methods=['GET', 'POST'])
@login_required
def delete_book():
    list_books_for = book.query.all()
    book_to_delete = None
